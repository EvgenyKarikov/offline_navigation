...
# Модель для depth (работает на панорамах, но undistort'те вручную для accuracy)
depth_model = pipeline("depth-estimation", model="depth-anything/Depth-Anything-V2-Large-hf")

image_processor = DepthProImageProcessorFast.from_pretrained("apple/DepthPro-hf")
model = DepthProForDepthEstimation.from_pretrained("apple/DepthPro-hf").to(device)

def fetch_yandex(lat, lon, num_neighbors=5, output_dir='yandex_images'):
    """
...
# Пример: Москва, центр (замените на off-road, e.g. Подмосковье)
lat, lon = 55.7558, 37.6173
fetched = fetch_yandex(lat, lon, num_neighbors=5)

print(f"Всего скачано: {len(fetched)} панорам")